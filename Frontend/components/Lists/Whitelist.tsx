...
export const Whitelist: React.FC = () => {
  ...
  const { data: whitelist, isLoading } = useQuery({
    queryKey: ['whitelist'],
    queryFn: () => window.electronAPI.getWhitelist(),
  });

  const addMutation = useMutation({
    mutationFn: async (domain: string) => {
      await window.electronAPI.addToWhitelist(domain);
    },
    onSuccess: () => {
      setNewDomain('');
  ...
  const removeMutation = useMutation({
    mutationFn: async (domain: string) => {
      await window.electronAPI.removeFromWhitelist(domain);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['whitelist'] });
      queryClient.invalidateQueries({ queryKey: ['stats'] });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if(!domainRegex.test(newDomain) && newDomain !== '127.0.0.1') {
        setIsError(true);
        return;
    }
  ...
  return (
    <div className="max-w-6xl mx-auto animate-fade-in pb-12 w-full">
      <header className="flex flex-col items-center justify-center mb-12 lg:mb-16 text-center px-4">
        <h1 className="text-3xl md:text-5xl font-light tracking-tight text-black mb-4 transition-colors">{t('whitelist.title')}</h1>
        <p className="text-secondary text-base md:text-lg font-light transition-colors">{t('whitelist.subtitle')}</p>
      </header>
      ...
      <div className="max-w-2xl mx-auto mb-16 px-4">
        <form onSubmit={handleSubmit} className="relative">
            ...
            {isError && <p className="absolute top-full mt-2 text-red-500 text-xs text-center sm:text-left w-full">{t('whitelist.error_domain')}</p>}
        </form>
      </div>
        ...
        <table className="w-full text-left border-collapse min-w-[600px]">
            <thead>
                <tr className="border-b border-border bg-subtle transition-colors">
                    <th className="p-4 text-xs font-medium text-secondary uppercase tracking-wider">{t('whitelist.table.domain')}</th>
                    <th className="p-4 text-xs font-medium text-secondary uppercase tracking-wider">{t('whitelist.table.source')}</th>
                    <th className="p-4 text-xs font-medium text-secondary uppercase tracking-wider text-right">{t('whitelist.table.date_added')}</th>
                    <th className="p-4 text-xs font-medium text-secondary uppercase tracking-wider text-right w-24">{t('whitelist.table.action')}</th>
                </tr>
            </thead>
            <tbody className="divide-y divide-border">
                {isLoading ? (
                    <tr><td colSpan={4} className="p-8 text-center text-secondary">{t('whitelist.loading')}</td></tr>
                ) : whitelist?.filter(e => e.source !== 'system').length === 0 ? (
                    <tr><td colSpan={4} className="p-8 text-center text-secondary">{t('whitelist.empty')}</td></tr>
                ) : (
                    whitelist?.filter(entry => entry.source !== 'system').map((entry) => (
